    ...
    const updateSolver = () => {
      if (newSolver) {
        ...
        let goal = {
          ...
          translation: [
              0.0,
              Math.cos(time / 1000) / 20 + 0.4, // The sin function alternating between -1 and 1 to have the robot to swing back and forth along the y-axis
        ...
        const newState = newSolver.jacobianIk(goal, {translation:[0.0,0.0,0.0],rotation:[0.0,0.0,0.0,1.0]}, robot === 'panda' ? 'panda_hand' : 'tool0'); // Pass the new goal into solve function
        if (newState?.frames) {
          // Update the solver's current state
          setRobotState(newState);
        }
