...
function InitializationExample(props) {
  ...
  const initialGoal = {
    position: { Line: {start: [0,0,0], end: [1,1,1]}, }
  };
  ...
  useEffect(() => {
    const updateSolver = () => {
      if (livelySolver) {
        ...
        let newTranslation = [Math.sin(time / 1000) / 5 + -0.8, 0.5, 0.25];
        let newTranslation2 = [0.8, 0.5, Math.sin(time / 500) / 10 + 0.3];
        newGoal = {
          position: { Line: {start: newTranslation, end: newTranslation2}, }
        };
        ...
        setEnvShapes([
          ...
          {
            type: "Box", //can be 'Cylinder', 'Capsule', or 'Sphere'
            name: "box2", // name can be arbitrary
            frame: "world", // or 'world'
            physical: false, // physical collision
            x: 0.25,
            y: 0.25,
  ...
  }, [livelySolver,shapesUpdate,newGoal]); // Update the solver 30fps},[robot])
