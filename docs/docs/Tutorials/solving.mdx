...
```py
from lively import Solver, Translation, Rotation, SmoothnessMacroObjective, CollisionAvoidanceObjective, JointLimitsObjective, PositionMatchObjective, OrientationMatchObjective, JointMatchObjective, BoxShape, CollisionSettingInfo, ScalarRange
...
solver = Solver(
  ...
  objectives={
      ...
      "smoothness":SmoothnessMacroObjective(name="MySmoothnessObjective",weight=20,joints=True,origin=False,links=True),
      "position": PositionMatchObjective(name="MyPositionMatchObjective", weight=15,link="panda_hand"),
      "orientation": OrientationMatchObjective(name="MyOrientationMatchObjective", weight=10,link="panda_hand"),
      "collision": CollisionAvoidanceObjective(name="MyCollisionAvoidanceObjective", weight=3),
      "jointLimit": JointLimitsObjective(name="MyJointLimitObjective", weight=5),
      "jointMatch": JointMatchObjective(name="MyJointMatchObjective", weight=5, joint="panda_finger_joint1")
  },
  root_bounds=[
...
state = solver.solve(
    goals= {
        "position": Translation(x=1.0, y =2.0, z=3.0),
        "orientation": Rotation(0.0,0.707,0.0,0.0),
        "jointMatch": 0.02
    },
    weights = {},
    time = 0.0
