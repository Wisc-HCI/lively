...
The `shape_update` parameter in `solve` takes in a list of `ShapeUpdate` objects where the environmental shapes can be introduced, modified, and removed.
...
:::note
The [`shape`](../../Shapes/) objects passed in the `shapes` parameter in `solver` [`initialization`](../initialization) are static and can not be moved or deleted later. Only the shapes that are introduced (`Add`) to the environment in `ShapeUpdate` can be modified and removed by `Move` and `delete`.
:::
