...
```rust
  ...
  let shape_update: Vec<ShapeUpdate> = vec![
      // This is an Add ShapeUpdate functionality or type. This introduce a new Cube with "add_box_1" as the id to the environment.
      ShapeUpdate::Add {
          id: add_box_1.to_string(),// must be an unique id
          shape: add_box.clone(),
      }
  ];
  solver.solve(
