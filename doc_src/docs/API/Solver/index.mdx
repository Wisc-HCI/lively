...
including the robot's URDF, a set of [`objectives`](../Objectives/objective), 
initial joint [`state`](../state), and a description of the collision environment.

It is optional, but advised, to then execute the [`compute_average_distance_table`](./Methods/compute_average_distance_table) 
