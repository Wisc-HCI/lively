...
:::note
If no values are provided for the `optional` field, the [`Solver`](../Solver) will just use the default values and behaviors for the `optional` parameters.
:::
...
solver = Solver(
  ...
  objectives={
      ...
      "SmoothnessMacroObjective":SmoothnessMacroObjective(name="General Smoothness",weight=10,joints=true, origin=false, links=false),
...
fn main(){
 ...
 let smooth_macro_obj = SmoothnessMacroObjective::new("Smoothness".to_string(), 30.0, true, false, false);
