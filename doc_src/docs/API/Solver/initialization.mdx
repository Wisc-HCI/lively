...
|`root_bounds`|list of (float,float)|yes|The list of hard limits on how much the base of the robot can move relative to the world frame|
|`shapes`|list of [`Shape`](../Shapes) objects|yes|The initial environmental shapes|
|`initial_state`|[`State`](../state) objects|yes|The initial state to the `Solver`|
|`max_retries`|integer|yes|The maximum number of randomly initialized rounds allowed for each invocation of `solve`|
|`max_iterations`|integer|yes|The number of maximum iterations per round within the optimization|
|`collision_settings`|[`CollisionSettingInfo`](../Info/CollisionSettingInfo)|yes|The initial setting to [`Collision Avoidance`](../collision)|
