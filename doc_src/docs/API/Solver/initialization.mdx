...
<Tabs>
  ...
  let solver = new Solver(
    ...
    objectives = { //The initial objectives to the solver
            "eePosition": {
    ...
    shapes = [{
        ...
        x:0.5,y:0.5,z:0.2, // dimension of the box
        localTransform: {translation:[0.0,0.0,0.0],rotation:[0.0,0.0,0.0,1.0]} // [x, y, z, w] ordering for quaternion
    }],
    initial_state= {origin:{translation:[0,0,0],rotation:[1,0,0,0]},joints:{panda_joint1:0.0,panda_joint2:0.0}}, //only the origin and joints need to be defined for the state object for initial_state
    ...
    collision_settings = {dMax : 0.3, r : 0.0, aMax : 2.0, timeBudget : 100, timed : true} // if not provided, the solve will use the default values
  );
  ```
