...
|---|---|---|---|---|
|`urdf`|string|no||The string representation of XML format. |
|`objectives`|look-up table of <string,[`Objective`](../Objectives)> key-value pairs |no||The initial objectives to the `Solver`.|
|`root_bounds`|list of tuple of (float,float)|yes|The root of the robot is fixated to the origin in the world frame.|The list of hard limits on how much the base of the robot can move relative to the world frame. Each element is a tuple of two float. The first float number is the center value and the second value is the delta|
|`shapes`|list of [`Shape`](../Shapes) objects|yes|There will be no other shapes beside that of the robot.|The initial environmental shapes.|
|`initial_state`|[`State`](../state) objects|yes||The initial state to the `Solver`.|
|`max_retries`|integer|yes|Value of 1 is used|The maximum number of randomly initialized rounds allowed for each invocation of `solve`.|
|`max_iterations`|integer|yes|value of 150 is used|The number of maximum iterations per round within the optimization.|
|`collision_settings`|[`CollisionSettingInfo`](../Info/CollisionSettingInfo)|yes|[`Default CollisionSettingInfo`](../Info/collisionSettingInfo)|The initial setting to [`Collision Avoidance`](../collision).|
