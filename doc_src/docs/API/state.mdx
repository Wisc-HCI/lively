...
|`joints`|lookup table(string, float)|no|A lookup table of the joint values for each movable joint|
|`frames`|lookup table(string, [`TransformInfo`](../API/Info/transformInfo))|can be ignored for [`Solver initialization`](docs/API/Solver/initialization.mdx)|A lookup table of each link’s position in both world and local coordinates|
