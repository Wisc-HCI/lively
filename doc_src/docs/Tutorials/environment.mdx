...
function InitializationExample(props) {
    ...
    const [controllerPosition, setControllerPosition] = useState(controller);
    
    function updateControllerPosition(transform){
        console.log("updateControllerpos" , transform);
        const newController = controller;
        newController.localTransform.translation = transform.position;
        setControllerPosition(newController);
    ...
    const shapes = [{
                    ...
                    x:0.25,y:0.25,z:0.25, // dimension of the box
                    localTransform: {translation:[0.6,0.05,0.15],rotation:[0.0,0.0,0.0,1.0]} // [x, y, z, w] ordering for quaternion
                },
                {
                    type:'Capsule',
                    name:'pill',
                    ...
                    length:0.5,
                    radius:0.2,
                    localTransform: {translation:[-0.5,0.0,0.5],rotation:[1.0,0.0,0.0,0.0]} // [x, y, z, w] ordering for quaternion
                },
                {
                    type:'Cylinder',
                    name:'zone',
                    ...
                    length:0.2,
                    radius:0.1,
                    localTransform: {translation:[0.0,0.5,0.3],rotation:[0.0,0.0,0.0,1.0]} // [x, y, z, w] ordering for quaternion
                },
    ...
    useEffect(()=>{
        ...
        */
        ...
        const updateSolver = () => {
            ...
            if (currentSolver) {
                const d = new Date();
                let time = d.getTime(); // Get the time used in milliseconds
            let goal = { // An goal example with defined Translation and Rotation
                      
            }
            const newState = currentSolver.solve({},{},0.0); // Pass the new goal into solve function 
            // Update the solver's current state
  ...
  return (
    <div>
       ...
       <RobotViewer state={robotState} links={livelySolver ? livelySolver.links : []} shapes = {shapes} controller = {controllerPosition} onMove={(id, source, worldTransform, localTransform)=>{updateControllerPosition(localTransform)}}/>
