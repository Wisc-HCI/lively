...
function InitializationExample(props) {
  const arrowTransformControl = {
    ...
    localTransform: {
      ...
      rotation: [1.0, 0.0, 0.0, 0.0], // The initial rotation is pointing to where the gripper is pointing at.
    },
  };
...
let mut solver = Solver::new(
  ...
  objectives,
  None,
  box_shapes_vec.clone() // static environmental shapes, cannot be updated.
);
...
let shape_update: Vec<ShapeUpdate> = vec![ShapeUpdate::Add {
            id: 1.to_string(),
            shape: box_2.clone(), //new "world"
        }];
