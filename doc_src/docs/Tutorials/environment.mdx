...
function InitializationExample(props) {
  ...
  const initialEnvShapes = [
    // the environmental models in the scene.
    /* {
  ...
  function addEnvCube(){
    const cube =  {
      Add : {
      id: "env-box-1" ,// must be an unique id
      shape : {type: "Box", //can be 'Cylinder', 'Capsule', or 'Sphere'
      name: "box", // name can be arbitrary
      frame: "world", // or 'world'
      physical: true, // physical collision
      x: 0.25,
    ...
    setEnvShapes([{type: "Box", //can be 'Cylinder', 'Capsule', or 'Sphere'
      name: "box", // name can be arbitrary
      frame: "world", // or 'world'
      physical: true, // physical collision
      x: 0.25,
 ...
 // console.log(shapesUpdate);
  function updateEnvironmentalShapesTransform(id, transform) {
    ...
    if (newShapeUpdate){
      setShapesUpdate({Move : {id: newShapeUpdate.name, transform: newShapeUpdate.localTransform  }});
    }
  ...
  useEffect(() => {
    ...
    const updateSolver = () => {
      if (currentSolver) {
        ...
        const newState = currentSolver.solve(goal, {}, time / 1000, activeCube ); // Pass the new goal into solve function
        // Update the solver's current state 
        setRobotState(newState);
      }
    };
  ...
  return (
    <div>
      ...
      <Button
        ...
        onClick={() => addEnvCube()}
      >
        {activeCube ? 'Delete environmental shape - Cube' : 'Add environmental shape - Cube'}
      </Button>
