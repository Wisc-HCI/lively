...
function InitializationExample(props) {
  ...
  const [controllerGoal, setControllerGoal] = useState(originalGoal);

  function updateControllerPosition(transform) {
    console.log("updatentransform" , transform);
    let newController = controllerPosition;
    newController.localTransform.translation = transform.position;
    let newGoal = controllerGoal;
    newGoal.position.Translation[0] = transform.position.x;
    newGoal.position.Translation[1] = transform.position.y;
    newGoal.position.Translation[2] = transform.position.z;
    //console.log("updatenewgoal" , newGoal);
    setControllerPosition(newController);
    setControllerGoal(newGoal);
  }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const newState = newSolver.solve(controllerGoal, {}, 0.0);
    ...
    const updateSolver = () => {
      if (currentSolver) {
        const newState = currentSolver.solve(controllerGoal, {}, 0.0); // Pass the new goal into solve function
        // Update the solver's current state
        setRobotState(newState);
      }
    };
    ...
    <div>
      <RobotViewer
        ...
        onMove={(id, source, worldTransform, localTransform) => {
          updateControllerPosition(localTransform);
        }}
      />
      <Button
        active={robot === "panda"}
