...
function InitializationExample(props) {
  const initialGoalValues = {
    ...
    translation: [-0.35289461347986373,0.13931635901132902,0.38946876678037234],
    // The initial rotation is pointing to where the gripper is pointing at.
    rotation: [0.968947708849441,0.01867797756065113,-0.2465018112869008,0.005322377470646942],
  };
  ...
  const initialState = {
    ...
    joints: {"shoulder_pan_joint":3.054197606864953,"elbow_joint":-1.2683111703953787,"wrist_3_joint":-4.823194599105927,"robotiq_85_left_finger_tip_joint":-0.051922213806626316,"robotiq_85_right_inner_knuckle_joint":0.051922213806626316,"robotiq_85_right_finger_tip_joint":-0.051922213806626316,"shoulder_lift_joint":-1.748886737780085,"wrist_1_joint":5.08275093442812,"robotiq_85_left_inner_knuckle_joint":0.051922213806626316,"robotiq_85_right_knuckle_joint":0.051922213806626316,"robotiq_85_left_knuckle_joint":0.051922213806626316,"wrist_2_joint":-4.651476380979644},
  };
  ...
  useEffect(() => {
    ...
    var currentSolver;
      ...
      const newState = newSolver.solve(goal, {}, 0.0,shapesUpdate);
    ...
    const updateSolver = () => {
      console.log("shapoesUpdate" , shapesUpdate);
      if (currentSolver) {
        
        const d = new Date();
        let time = d.getTime(); // Get the time in milliseconds
        const newState = currentSolver.solve(
          goal,
          {},
          time / 1000,
  ...
  }, [robot,shapesUpdate]); // Rerun this code if the robot changes
...
async function start() {
  ...
  let state = solver.solve({}, {}, time, shape_update);
