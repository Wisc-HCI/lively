...
function InitializationExample(props) {
  ...
  const [torsoTranslation, setTorsoTranslation] = useState([0,0,0.4])
  ...
  return (
    <div>
      <RobotViewer
        ...
        levaOptions={{
          torsoTranslation:{
            label: "Torso Translation",
            value: torsoTranslation,
            onChange: (v) => {setTorsoTranslation(v)}
          }
        }}
      />
