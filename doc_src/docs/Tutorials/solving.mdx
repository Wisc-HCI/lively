        ...
        const updateSolver = () => {
            ...
            'position':{
                Translation :[
                    0.0,
                    0.5,
                    Math.sin(time/1000)/4+0.75 // The sin function alternating between -1 and 1 to have the robot to swing back and forth along the z-axis
                ]
                },
            'orientation':{
            ...
            const newState = currentSolver.solve(goal,{},0.0); // Pass the new goal into solve function 
            // Update the solver's current state
            setRobotState(newState)
            } 
