  ...
  useEffect(() => {
    ...
    const updateSolver = () => {
      if (currentSolver) {
        ...
        const newState = currentSolver.solve(goals, {}, time / 1000); // Convert the time to second
        // Update the solver's current state
...
objectives.insert(
  ...
  Objective::PositionLiveliness(PositionLiveliness::new("MyLivelinessObjective",25,"panda_hand",7))
  Objective::OrientationLiveliness(OrientationLiveliness::new("MyOrientationlinessObjective",15,"panda_link2",7))
);
