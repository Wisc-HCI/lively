...
use nalgebra::geometry::{Point3, UnitQuaternion, Quaternion, Isometry3, Translation3};
...
impl MacroSmoothness {
    pub fn new() -> Self {Self{velocity_objective:MinimizeVelocity,
                               acceleration_objective:MinimizeAcceleration,
                               jerk_objective:MinimizeJerk}}
}
...
impl Gravity {
    pub fn new(indices: Vec<usize>) -> Self {Self{arm_idx:indices[0], joint_idx:indices[1]}}
}
...
impl PositionBounding  {
    pub fn new(goal_idx: usize, shape: Vector3<f64>, indices: Vec<usize>) -> Self {Self{goal_idx, arm_idx:indices[0], joint_idx:indices[1], shape}}
}
    ...
    fn call(&self, x: &[f64], v: &RelaxedIKVars, frames: &Vec<(Vec<Vector3<f64>>, Vec<UnitQuaternion<f64>>)>, _is_core: bool) -> f64 {
        ...
        match v.goals[self.goal_idx].value {
            ...
            Goal::Pose(pose_pair) => {
                ...
                x_val = (pos[0].powi(1)/self.shape[0].powi(2) + pos[1].powi(2)/self.shape[1].powi(2) + pos[2].powi(2)/self.shape[2].powi(2)).powi(2)
            },
            // If there is no goal, assume it is the position/orientation from core
            Goal::None => {
                let pose_pair = (v.frames_core[self.arm_idx].0[self.joint_idx],v.frames_core[self.arm_idx].1[self.joint_idx]);
                // Translate position into coordinate frame of ellipse based on pose_pair
                let transform = Isometry3::from_parts(Translation3::from(pose_pair.0),pose_pair.1);
                let pos = transform.inverse_transform_point(&position);
