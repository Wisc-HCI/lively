        ...
        for objective in objectives {
            ...
            match objective.variant {
                ObjectiveVariant::PositionLiveliness => {
                    ...
                    seeds.push(Goal::Vector(Vector3::new(
    ...
    pub fn update(&mut self, time: f64) {
        for i in 0..self.goals.len() {
            match (self.goals[i], self.seeds[i]) {
                ...
                (Goal::Scalar(_goal), Goal::Scalar(seed)) => {
                    self.goals[i] = Goal::Scalar(
                        self.sizes[i]
                            * self.perlin.get([
                                time / self.freqs[i],
                                seed,
                                500.0 * ((time / self.freqs[i] + seed) / 500.0).sin(),
                            ]),
                    );
                }
                // If the goal is a 3-vector
                (Goal::Vector(_goal), Goal::Vector(seed)) => {
                    self.goals[i] = Goal::Vector(Vector3::new(
                        self.shapes[i][0]
                            * self.perlin.get([
                                time / self.freqs[i],
                                seed[0],
                                500.0 * ((time / self.freqs[i] + seed[0]) / 500.0).sin(),
                            ]),
                        self.shapes[i][1]
                            * self.perlin.get([
                                time / self.freqs[i],
                                seed[1],
                                500.0 * ((time / self.freqs[i] + seed[1]) / 500.0).sin(),
                            ]),
                        self.shapes[i][2]
                            * self.perlin.get([
                                time / self.freqs[i],
                                seed[2],
                                500.0 * ((time / self.freqs[i] + seed[2]) / 500.0).sin(),
                            ]),
                    ));
                }
