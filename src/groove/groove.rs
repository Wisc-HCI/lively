...
impl OptimizationEngineOpen {
    ...
    pub fn optimize(
        ...
        let bounds = Rectangle::new(
            Option::from(v.robot.lower_bounds.as_slice()),
            Option::from(v.robot.upper_bounds.as_slice()),
        );
...
impl OptimizationEngineNLopt {
    ...
    pub fn optimize(
        ...
        opt.set_ftol_rel(0.000001).unwrap();
        opt.set_maxeval(max_iter).unwrap();

        // let mut x_init = x_out.to_vec();
        let _res = opt.optimize(x_out);
