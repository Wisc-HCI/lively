            ...
            for collision in &link.collisions{
                ...
                match geom_element { //parsing each link shapes.
                    ...
                    k::link::Geometry::Capsule{radius,length} => {
                        let point_a = Point::new(*length  * vector![0.0,1.0,0.0][0],*length  * vector![0.0,1.0,0.0][1],*length*vector![0.0,1.0,0.0][2] );
                        let point_b = Point::new(*length*vector![0.0,-1.0,0.0][0],*length*vector![0.0,-1.0,0.0][1],*length*vector![0.0,-1.0,0.0][2]);
            ...
            match update {
                ShapeUpdate::Add{id,shape} => {
                    match shape {
                        ...
                        shapes::Shape::Cylinder(cylinder_object) => {
                            ...
                            let new_length = cylinder_object.length / 2.0;
                            let cylinder_collider = ColliderBuilder::new(SharedShape::cylinder(new_length , cylinder_object.radius ))
                ...
                ShapeUpdate::Move{id,pose}=>{
                    for (transient_id,transient_handle) in &mut self.transient_group {
                        if transient_id == id {
                            let transient_collider = self.link_collider_set.get_mut(*transient_handle);
                            ...
                            Ok (valid_closest_points) =>{
                                match valid_closest_points {
                                    ClosestPoints::Intersecting => {
                                        let shape_name1 = self.shape_name_look_up.get(&handle1).unwrap();
                                        let shape_name2 = self.shape_name_look_up.get(&handle2).unwrap();
