...
impl CollisionManager {
    pub fn new(chain : Chain<f64>, persistent_shapes: Vec<shapes::Shape>) -> Self {
        ...
        let mut collider_changed : Vec<ColliderHandle> = vec![];
        let mut transient_group : Vec<(String,ColliderHandle)> = vec![];
        ...
        for link in chain.iter_links(){
            ...
            for collision in &link.collisions{
                ...
                match geom_element { //parsing each link shapes.
                    k::link::Geometry::Cylinder{radius,length} =>{  
                        let newLength = *length / 2.0;
                        let cylinder_shape = SharedShape::cylinder(newLength , *radius);
                    ...
                    k::link::Geometry::Capsule{radius,length} => {
                        let mut point_a = Point::new(*length  * vector![0.0,1.0,0.0][0],*length  * vector![0.0,1.0,0.0][1],*length*vector![0.0,1.0,0.0][2] );
                        let mut point_b = Point::new(*length*vector![0.0,-1.0,0.0][0],*length*vector![0.0,-1.0,0.0][1],*length*vector![0.0,-1.0,0.0][2]);
        ...
        for shape in  &persistent_shapes { 
            match shape {
                shapes::Shape::Box(box_object) => {
                    ...
                    let frame_existing_collider_handle = collider_handle_look_up.get_mut(&box_object.frame);              
                    match frame_existing_collider_handle{
                        Some(collider_handle) => {
                            match (collider_vector_look_up.get_mut(&box_object.frame)){
                                Some(collider_vector) => {
                                    collider_vector.push((box_object.local_transform,box_shape.clone()));
                                    ...
                                    match (link_collider_set.remove(*collider_handle, &mut island_manager, &mut robot_rigid_body_set,false)){
                                        Some(collider) => {
                                            ...
                                            link_group.push((box_object.frame.to_string(),new_handle.clone()));
                                            shape_name_look_up.insert(new_handle.clone(),box_object.frame.to_string());
            ...
            shapes::Shape::Cylinder(cylinder_object) => {
                    let newLength = (cylinder_object.length / 2.0);
                    let cylinder_shape = SharedShape::cylinder(newLength, cylinder_object.radius); 
            ...
            shapes::Shape::Capsule(capsule_object) => {
                    let mut point_a = Point::new(capsule_object.length * vector![0.0,1.0,0.0][0],capsule_object.length  * vector![0.0,1.0,0.0][1],capsule_object.length * vector![0.0,1.0,0.0][2] );
                    let mut point_b = Point::new(capsule_object.length * vector![0.0,-1.0,0.0][0],capsule_object.length   * vector![0.0,-1.0,0.0][1],capsule_object.length * vector![0.0,-1.0,0.0][2]);
                    let capsule_shape = SharedShape::capsule(point_a,point_b ,capsule_object.radius);
                    let frame_existing_collider_handle = collider_handle_look_up.get_mut(&capsule_object.frame); 
                    match frame_existing_collider_handle{
                        Some(collider_handle) => {
                            match (collider_vector_look_up.get_mut(&capsule_object.frame)){
                                Some(collider_vector) => {
                                    collider_vector.push((capsule_object.local_transform,capsule_shape.clone()));
                                    let new_collider = ColliderBuilder::compound(collider_vector.to_vec()).active_events(ActiveEvents::CONTACT_EVENTS).user_data(0).build();
                                    match (link_collider_set.remove(*collider_handle, &mut island_manager, &mut robot_rigid_body_set,false)){
                                        Some(collider) => {
                                            let new_handle = link_collider_set.insert(new_collider);
                                            link_group.push((capsule_object.frame.to_string(),new_handle.clone()));
                                            shape_name_look_up.insert(new_handle.clone(),capsule_object.frame.to_string());
                        ...
                        None => {
                            let capsule_collider = ColliderBuilder::new(capsule_shape).position(capsule_object.local_transform).active_events(ActiveEvents::CONTACT_EVENTS).user_data(1).build();
            ...
            match update {
                ShapeUpdate::Add{id,shape} => {
                    match shape {
                        shapes::Shape::Box(box_object) => {
                            let mut physical = 1;
                            if (box_object.physical == true){
                                physical = 2;
                            }

                            let box_collider = ColliderBuilder::new(SharedShape::cuboid(box_object.y ,box_object.x ,box_object.z ))
                        ...
                        shapes::Shape::Cylinder(cylinder_object) => {
                            let mut physical = 1;
                            if (cylinder_object.physical == true){
                                physical = 2;
                            }
                            let newLength = (cylinder_object.length / 2.0);
                            let cylinder_collider = ColliderBuilder::new(SharedShape::cylinder(newLength , cylinder_object.radius ))
                            .position(cylinder_object.local_transform ).active_events(ActiveEvents::CONTACT_EVENTS).user_data(physical).build();
                        ...
                        shapes::Shape::Sphere(sphere_object) => {
                            let mut physical = 1;
                            if (sphere_object.physical == true){
                                physical = 2;
                            }
                            let sphere_collider = ColliderBuilder::new(SharedShape::ball(sphere_object.radius ))
                            .position(sphere_object.local_transform ).active_events(ActiveEvents::CONTACT_EVENTS).user_data(physical).build();
                        ...
                        shapes::Shape::Capsule(capsule_object) => {
                            let mut physical = 1;
                            if (capsule_object.physical == true){
                                physical = 2;
                            }
                            let mut point_a = Point::new(capsule_object.length  * vector![0.0,1.0,0.0][0],capsule_object.length  * vector![0.0,1.0,0.0][1],capsule_object.length   * vector![0.0,1.0,0.0][2] );
                            let mut point_b = Point::new(capsule_object.length   * vector![0.0,-1.0,0.0][0],capsule_object.length   * vector![0.0,-1.0,0.0][1],capsule_object.length  * vector![0.0,-1.0,0.0][2]);
                            let capsule_collider = ColliderBuilder::new(SharedShape::capsule(point_a,point_b ,capsule_object.radius ))
                            .position(capsule_object.local_transform ).active_events(ActiveEvents::CONTACT_EVENTS).user_data(physical).build();
                ...
                ShapeUpdate::Move{id,pose}=>{
                    for (transient_id,transient_handle) in &mut self.transient_group{
                        if (transient_id == id){
                            let mut transient_collider = self.link_collider_set.get_mut(*transient_handle);
                ...
                ShapeUpdate::Delete(id) =>{
                    ...
                    for (transient_id, transient_handle) in &mut self.transient_group{
                        if (transient_id == id){
                            let removed = self.link_collider_set.remove(*transient_handle,&mut self.island_manager,
                                                                         &mut self.robot_rigid_body_set,false);
                            match removed {
    ...
    pub fn get_proximity(&self, frames: &HashMap<String, Isometry3<f64>>) -> Vec<ProximityInfo> {
        ...
        let mut new_shape_name_look_up = self.shape_name_look_up.clone();
        

        for (name,group_handle) in &mut new_link_group{
            println!("{}" , name);
            let mut link_collider = new_link_collider_set.get_mut(*group_handle);
            match(link_collider){
        ...
        for (name,group_handle) in &mut new_transient_group{
            let mut transient_collider = new_link_collider_set.get_mut(*group_handle);
                match(transient_collider){
                    Some(matched_transient_collider) => {
                        let transform = frames.get(name);
                        match(transform){
        ...
        let mut pair_event_vec = Vec::<BroadPhasePairEvent>::new();
        new_broad_phase.update(1.0, &mut new_link_collider_set, &new_collider_changed[0..new_collider_changed.len()-1], &[],&mut pair_event_vec);
            ...
            match(collider1,collider2){
                (Some(first), Some(second)) => {
                    if (first.user_data == 0 && second.user_data == 0){
                        ...
                        match parry3d_f64::query::closest_points(&shape_collider_1_pos,shape1,&shape_collider_2_pos,shape2,1.0) {
                            Ok (valid_closest_points) =>{
                                match (valid_closest_points){
                                    ClosestPoints::Intersecting => {
                                        let shape_name1 = new_shape_name_look_up.get(&handle1).unwrap();
                                        let shape_name2 = new_shape_name_look_up.get(&handle2).unwrap();
                                        result_vector.push((ProximityInfo::new(shape_name1.to_string(),shape_name2.to_string(),0.0,None,true)));
                                    ...
                                    ClosestPoints::WithinMargin(point1,point2) => {
                                        let shape_name1 = new_shape_name_look_up.get(&handle1).unwrap();
                                        let shape_name2 = new_shape_name_look_up.get(&handle2).unwrap();
                                        let dist = ((point1.x-point2.x)*(point1.x-point2.x) + (point1.y-point2.y)*(point1.y-point2.y) 
                                                        + (point1.z-point2.z)*(point1.z-point2.z) as f64).sqrt();
                                        result_vector.push((ProximityInfo::new(shape_name1.to_string(),shape_name2.to_string(),dist,Some((point1,point2)),true)));
                            ...
                            Ok (valid_closest_points) =>{
                                match (valid_closest_points){
                                    ClosestPoints::Intersecting => {
                                        let shape_name1 = new_shape_name_look_up.get(&handle1).unwrap();
                                        let shape_name2 = new_shape_name_look_up.get(&handle2).unwrap();
                                        result_vector.push((ProximityInfo::new(shape_name1.to_string(),shape_name2.to_string(),0.0,None,false)));
                                    ...
                                    ClosestPoints::WithinMargin(point1,point2) => {
                                        ...
                                        let dist = ((point1.x-point2.x)*(point1.x-point2.x) + (point1.y-point2.y)*(point1.y-point2.y) 
                                                        + (point1.z-point2.z)*(point1.z-point2.z) as f64).sqrt();
                                        result_vector.push((ProximityInfo::new(shape_name1.to_string(),shape_name2.to_string(),dist,Some((point1,point2)),false)));
                                        // let dist = distance(point1,point2);
                                        // infos.push(ProximityInfo::new(shape1_name,shape2_name,Some(dist),Some((point1,point2)),is_physical))
                                    },
                                    ClosestPoints::Disjoint => {
